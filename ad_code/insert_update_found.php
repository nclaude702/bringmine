...
try {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (mysqli_stmt_execute($update)) {
            echo "<script>alert('Record updated successfully!'); window.location='../ad_found.php';</script>";
        } else {
            echo "<script>alert('Failed to update the record. Please try again.'); window.location='..ad_/found.php';</script>";
        }
    } else {
        // Insert a new record
        ...
        if (mysqli_stmt_execute($insert)) {
            echo "<script>alert('Record saved successfully!'); window.location='../ad_found.php';</script>";
        } else {
            echo "<script>alert('Failed to save the record. Please try again.'); window.location='../ad_found.php';</script>";
        }
    }
} catch (Exception $e) {
